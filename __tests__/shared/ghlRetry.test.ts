...
} from '../../amplify/functions/shared/ghlRetry';

// Retry policy behind the GHL client interceptor. Getting `isRetryableGhlStatus` wrong is costly in
...
describe('isRetryableGhlStatus', () => {
  it('retries rate limiting', () => {
    expect(isRetryableGhlStatus(429)).toBe(true);
  });

  it('retries transient server errors', () => {
    for (const status of [500, 502, 503, 504]) {
      expect(isRetryableGhlStatus(status)).toBe(true);
    }
  });

  it('does NOT retry client errors — the request itself is wrong', () => {
    for (const status of [400, 401, 403, 404, 409, 422]) {
      expect(isRetryableGhlStatus(status)).toBe(false);
    }
  });

  it('does NOT retry success codes', () => {
    expect(isRetryableGhlStatus(200)).toBe(false);
    expect(isRetryableGhlStatus(201)).toBe(false);
  });

  it('does NOT retry when there is no status (request never dispatched)', () => {
    expect(isRetryableGhlStatus(null)).toBe(false);
    expect(isRetryableGhlStatus(undefined)).toBe(false);
  });
});

describe('getGhlRetryDelayMs', () => {
  it('honours a numeric Retry-After, converting seconds to ms', () => {
    expect(getGhlRetryDelayMs(0, '2')).toBe(2000);
  });
  ...
  it('honours an HTTP-date Retry-After relative to now', () => {
    ...
    const delay = getGhlRetryDelayMs(0, soon, now);
  ...
  it('treats a past Retry-After date as retry-now, never a negative sleep', () => {
    ...
    expect(getGhlRetryDelayMs(0, past, now)).toBe(0);
  });
  ...
  it('clamps an absurd Retry-After to the cap so a retry cannot outlive the Lambda', () => {
    ...
    expect(getGhlRetryDelayMs(0, '600')).toBe(GHL_RETRY_MAX_DELAY_MS);
  });

  it('falls back to backoff when Retry-After is absent or unparseable', () => {
    for (const header of [null, undefined, '', '   ', 'soon', 'NaN']) {
      const delay = getGhlRetryDelayMs(0, header as any);
      expect(delay).toBeGreaterThan(0);
      expect(delay).toBeLessThanOrEqual(GHL_RETRY_BASE_MS);
    }
  });
  ...
  it('grows the backoff ceiling with each attempt', () => {
    ...
    const maxOver = (attempt: number) =>
      Math.max(...Array.from({ length: 200 }, () => getGhlRetryDelayMs(attempt)));
  ...
  it('never exceeds the cap however high the attempt count goes', () => {
    for (const attempt of [3, 5, 10, 50]) {
      expect(getGhlRetryDelayMs(attempt)).toBeLessThanOrEqual(GHL_RETRY_MAX_DELAY_MS);
    }
  });

  it('treats a negative attempt as attempt 0 rather than producing a tiny delay', () => {
    const delay = getGhlRetryDelayMs(-3);
    expect(delay).toBeGreaterThan(0);
    expect(delay).toBeLessThanOrEqual(GHL_RETRY_BASE_MS);
  });
  ...
  it('jitters so concurrent Lambdas do not retry in lockstep', () => {
    ...
    const samples = new Set(Array.from({ length: 100 }, () => getGhlRetryDelayMs(2)));
    expect(samples.size).toBeGreaterThan(1);
  });

  it('keeps jittered delays within [ceiling/2, ceiling]', () => {
    const ceiling = Math.min(GHL_RETRY_BASE_MS * 2 ** 2, GHL_RETRY_MAX_DELAY_MS);
    for (let i = 0; i < 200; i++) {
      const delay = getGhlRetryDelayMs(2);
