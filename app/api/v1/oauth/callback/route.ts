...
export async function GET(req: Request) {
  ...
  try {
    ...
    if (error) {
      ...
      return NextResponse.redirect(`${origin}/oauth/error?error=` + error);
    }

    if (!code) {
      console.error('No authorization code received');
      return NextResponse.redirect(`${origin}/oauth/error?error=no_code`);
    }
    ...
    return NextResponse.redirect(`${origin}/oauth/error?error=token_exchange_failed`);
  }
}
