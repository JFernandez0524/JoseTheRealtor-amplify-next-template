...
export default function EmailAnalytics() {
  ...
  async function loadStats() {
    try {
      ...
      const totalSent = queue.reduce((sum, q) => sum + (q.emailAttempts || 0), 0);

      const withEmails = queue.filter(q => q && q.lastEmailSent);
      
      const last7Days = withEmails
        .filter(q => q && q.lastEmailSent && new Date(q.lastEmailSent) >= sevenDaysAgo)
        .reduce((sum, q) => sum + (q.emailAttempts || 0), 0);

      const last30Days = withEmails
        .filter(q => q && q.lastEmailSent && new Date(q.lastEmailSent) >= thirtyDaysAgo)
        .reduce((sum, q) => sum + (q.emailAttempts || 0), 0);
