...
interface PricingClientProps {
  initialUserAccount: UserAccount | null;
  hasPaidPlan: boolean;
  isAI: boolean;
  userId: string | null;
}
    ...
    <div className='bg-slate-50 min-h-screen py-16 px-6'>
      <div className='max-w-6xl mx-auto'>
            ...
            return (
              <div
                ...
                className={`bg-white p-10 rounded-[2.5rem] border ${
                  isCurrent
                    ? 'border-indigo-500 ring-2 ring-indigo-50 shadow-lg'
                    : 'border-slate-200'
                } shadow-sm flex flex-col hover:border-indigo-300 transition-colors`}
              >
        ...
        <div className='bg-indigo-600 rounded-[3rem] p-8 md:p-16 text-white shadow-2xl relative overflow-hidden'>
          <HiOutlineQueueList className='absolute -bottom-20 -right-20 text-[25rem] text-indigo-500 opacity-20 pointer-events-none' />

          <div className='relative z-10'>
            <div className='flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12'>
              <div className='max-w-xl'>
                <h2 className='text-3xl font-black mb-4 flex items-center gap-3 tracking-tight'>
                  <HiOutlineBolt className='text-yellow-400' /> Skip-Trace
                  Wallet
                </h2>
